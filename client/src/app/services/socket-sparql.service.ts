        ...
        if (this.themeActive) {
            ...
            switch (this.activeFilters.filterAppreciation) {
                case 'highlyAppreciated':
                    const query_lurelu = SPARQL_QUERY_LURELU_FILTER(`FILTER(?isbn IN (${isbns}))`);
                    const query_babelio = SPARQL_BABELIO(`FILTER(?averageReview >= 4 && ?isbn IN (${isbns}))`);
                    const query_bnf = SPARQL_QUERY_BNF(`FILTER((?avis = "Coup de coeur !" || ?avis = "Bravo !") && ?isbn IN (${isbns}))`);
                    const query_constellations = SPARQL_QUERY_CONSTELLATIONS(`FILTER(?isCoupDeCoeur = true && ?isbn IN (${isbns}))`);
    ...
    updateBook() {
        ...
        switch (this.activeFilters.filterAppreciation) {
            case 'highlyAppreciated':
                ...
                const query_bnf = SPARQL_QUERY_BNF(`FILTER(?avis = "Coup de coeur !" || ?avis = "Bravo !")`);
                const query_constellations = SPARQL_QUERY_CONSTELLATIONS(`FILTER(?isCoupDeCoeur = true)`);
